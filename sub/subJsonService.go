...
func (s *SubJsonService) realityData(rData map[string]any, client model.Client) map[string]any {
	...
	rltyData["shortId"] = fmt.Sprintf("%s%s", u[len(u)-2], u[len(u)-1])
...
func (s *SubJsonService) genVless(inbound *model.Inbound, streamSettings json_util.RawMessage, client model.Client, ip_link string) json_util.RawMessage {
	outbound := Outbound{}
	outbound.Protocol = string(inbound.Protocol)
	//outbound.Tag = "proxy"
